...
$(function(){
  ...
  function drawFileCharts() {
    ...
    $('.js-file-chart').each(function(){
      ...
      value = el.data('difficulty');
      el.append(horizontalBar(value, Math.min(value * 2, width), 'difficulty', [5,10]));
  ...
  function drawOverviewCharts(reports) {
    ...
    reports.forEach(function(report){
      ...
      difficulty.data.push({
        value : report.complexity.aggregate.complexity.halstead.difficulty.toFixed(2),
        label : report.info.fileShort
      });
    });
